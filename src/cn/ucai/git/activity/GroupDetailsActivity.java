...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		groupMembers = SuperWeChatApplication.getInstance().getGroupmember().get(groupId);
		...
		registerGroupMmberChangedReceiver();
	}
						...
						try {
							...
							runOnUiThread(new Runnable() {
								public void run() {
								    refreshMembers();
									progressDialog.dismiss();
									Toast.makeText(getApplicationContext(), stsuccess, Toast.LENGTH_LONG).show();
								}
							});
						} catch (EaseMobException e) {
							runOnUiThread(new Runnable() {
	...
	private void refreshMembers(){
		String groupTitle = mGroup.getMGroupName() + "(" + mGroup.getMGroupAffiliationsCount() + ")";
		((TextView) findViewById(R.id.group_name)).setText(groupTitle);
		ArrayList<Member> list = SuperWeChatApplication.getInstance().getGroupmember().get(groupId);
		adapter.clear();
		ArrayList<Member> members = new ArrayList<Member>();
		members.addAll(list);
...
//				button.setCompoundDrawables(null, avatar, null, null);
				...
				UserUtils.setUserBeanAvatar(member.getMUserName(),holder.imageView);
	...
	class GroupMemberChangedReceiver extends BroadcastReceiver {
	...
	GroupMemberChangedReceiver mReceiver;
	private void registerGroupMmberChangedReceiver() {
		mReceiver = new GroupMemberChangedReceiver();
